...
// Version: 0.1.5 - Preserved experimental mode (11 rounds, no collapse)
 ...
 * @returns {Object} - {success: boolean, error?: string}
  ...
  calculateScorePreview(playerId, diceIds) {
    ...
    // Use centralized scoring system for preview
    const preview = this.scoringSystem.getScorePreview(player, diceIds);
  ...
  /**
   * Player flees the factory (Not available in v0.1.5 - no collapse mechanic)
   * @param {string} playerId - Player ID
   * @returns {Object} - {success: boolean, error?: string}
   */
  fleeFatory(playerId) { // Note: keeping typo for compatibility
    return { success: false, error: 'Flee action not available in v0.1.5' };
  }
 ...
 * @returns {Object} - Result
  ...
  processEndOfTurn() {
    ...
    // Process factory triggers at turn end
